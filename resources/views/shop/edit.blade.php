...
// });
	...
	@endif
<form id="regForm" action="{{ route('shops.update',$shop->id) }}" method="post" enctype="multipart/form-data">
	  @csrf
     @method('PUT')	 
				  ...
				  <div class="row">
					  ...
					  <div class="col-sm-6 text-left">
						  <select id="township_id" class="form-control" name="township_id">
							  <option value="{{$shop->township_id}}">{{$township}}</option>
						  </select>
					  </div>
				  </div> <br>
						  <div class="row">
							  ...
							  <div class="col-sm-6 text-left">
								  <textarea class="form-control form-control-alternative" rows="3" placeholder="အသေးစိတ်ထည့်ရန်" id='address' name="loacationaddress">{{$shop->loacationaddress}}</textarea>
							  </div>
						  </div>
	  		...
	  		<div class="row">
	  			<div class="col-sm-5 text-center">
					  ...
					  <label for="datepicker">အလုပ်ရုံစတင်လုပ်ကိုင်သောရက်စွဲ</label>
		  			  <input type="date" name="StartDate" id="datepicker" class="datepicker" value="{{ $shop->StartDate}}">
	  			</div>
	  			<div class="col-sm-1">
